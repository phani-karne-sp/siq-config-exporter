...
namespace SiqConfigExport
{
    public class Data
    {
        ...
        //TODO why are all these methods static?
        ...
        public static DataSet GetDataFromSIQdb(string connectionString)
        {
            ...
            using (SqlConnection sqlCn = new SqlConnection(connectionString))
            {
                ...
                {
                    //TODO we had a hard-coded DB name in one of the queries in the settings files...is that right?
                    DataTable dt = getDataTable(sqlCn, item.Name, (string)item.DefaultValue);
                    ds.Tables.Add(dt);
                }

                //ds.Tables.Add(getDataTable(sqlCn, 
                //       Queries.Default.Properties. SqlStmtKeys.DB_INFO1,
